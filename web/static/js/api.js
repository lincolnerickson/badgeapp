...
const API = {
    ...
    async saveCSV() {
        const resp = await fetch('/api/csv/save', { method: 'POST' });
        return resp.json();
    },
    ...
    async downloadTemplate() {
        const resp = await fetch('/api/download-template');
        if (!resp.ok) throw new Error('Download failed');
        return resp.blob();
    },
    ...
    async deleteField(idx) {
        const resp = await fetch(`/api/fields/${idx}`, { method: 'DELETE' });
        return resp.json();
    },
    ...
    async getCSVInfo() {
        const resp = await fetch('/api/csv/info');
        return resp.json();
    },
    ...
    async deleteRow(idx) {
        const resp = await fetch(`/api/csv/row/${idx}`, { method: 'DELETE' });
        return resp.json();
    },

    async searchCSV(query) {
        const resp = await fetch(`/api/csv/search?q=${encodeURIComponent(query)}`);
        return resp.json();
    },

    async getNextBadgeNumber(column) {
        const resp = await fetch(`/api/csv/next-badge-number?column=${encodeURIComponent(column)}`);
        return resp.json();
    },
    ...
    getPreviewURL(rowIdx) {
        return `/api/preview/${rowIdx}?t=${Date.now()}`;
    },

    getPreviewCustomURL(values) {
        return `/api/preview-custom?values=${encodeURIComponent(JSON.stringify(values))}&t=${Date.now()}`;
    },

    getBackgroundURL() {
        return `/api/background-image?t=${Date.now()}`;
    },
    ...
    async getPDFStatus(taskId) {
        const resp = await fetch(`/api/export-pdf/status/${taskId}`);
        return resp.json();
    },

    getPDFDownloadURL(taskId) {
        return `/api/export-pdf/download/${taskId}`;
    },

    getSinglePDFURL(rowIdx) {
        return `/api/export-single-pdf/${rowIdx}`;
    },

    getSingleImageURL(rowIdx) {
        return `/api/export-single-image/${rowIdx}`;
    },
